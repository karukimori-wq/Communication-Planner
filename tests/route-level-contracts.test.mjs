...
describe("route-level contract guards", () => {
  ...
  it("keeps channel message ingestion route validating required fields before success", () => { const route=read("src/app/api/channel-events/messages/route.ts"), store=read("src/lib/store.ts"); assert.match(route,/INVALID_JSON/); assert.match(route,/requireString\(body\.workspaceId, "workspaceId"\)/); assert.match(route,/normalizeChannel/); assert.match(route,/normalizeDirection/); assert.match(store,/findOrCreatePerson/); assert.match(store,/appendContextInsights/); });
