...
describe("provider live gate contracts",()=>{
 ...
 it("keeps provider send routed through readiness, rate-limit, dry-run and error guards",()=>{const s=read("src/lib/adapters/send.ts");assert.match(s,/getProviderSendReadiness/);assert.match(s,/checkProviderRateLimit/);assert.match(s,/deliveryMode: readiness\.effectiveDeliveryMode/);assert.match(s,/if \(!rateLimit\.ok\)/);assert.match(s,/httpStatus: 429/);assert.match(s,/mapProviderError/);});
});
