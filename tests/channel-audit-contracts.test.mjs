...
describe("channel audit contract guards", () => {
  it("validates channel event channel and direction before ingestion", () => {
    ...
    assert.match(route, /function normalizeChannel/); assert.match(route, /function normalizeDirection/);
    assert.match(route, /UNSUPPORTED_CHANNEL/); assert.match(route, /UNSUPPORTED_DIRECTION/);
    assert.match(route, /const input = \{ workspaceId, channel, externalUserId,/);
    assert.match(route, /body: messageBody, direction,/);
    assert.match(route, /ingestChannelMessage\(input\)/);
    assert.match(route, /D1CommunicationRepository\(db\)\.ingestChannelMessage/);
    assert.match(d1, /channel: "line" \| "x" \| "instagram"/);
    assert.match(d1, /direction: "inbound" \| "outbound"/);
  ...
  it("records send decision channel and adapter delivery as immutable audit input", () => {
    ...
    assert.match(types, /channel\?: Channel/); assert.match(types, /channelConfirmed: true/); assert.match(types, /adapterDelivery: \{/);
    assert.match(store, /adapterDelivery: ReplySendDecision\["adapterDelivery"\]/); assert.match(store, /channel: input\.channel/); assert.match(store, /adapterDelivery: input\.adapterDelivery/); assert.match(store, /channelConfirmed: true/);
    ...
    assert.doesNotMatch(sendRoute, /sendDecision\.channel = conversation\.channel/); assert.doesNotMatch(sendRoute, /sendDecision\.checks\.channelConfirmed = true/);
