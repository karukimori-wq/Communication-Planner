...
| `reply_send_decisions` | Reply-draft-scoped audit history for successful send gates |
...
- A send decision must keep `workspace_id + person_id + conversation_id + reply_draft_id + safety_check_id + message_id`.
