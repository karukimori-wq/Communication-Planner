...
| 6 | Provider production gate | Credentials, signature verification, provider verification flags, rate-limit policy, and provider error mapping are not yet production-configured | Keep live provider send blocked behind `/api/adapters/readiness`; fallback remains `dry_run` |
