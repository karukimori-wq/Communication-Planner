...
function readEnvelope<T>(value: unknown): SuccessEnvelope<T> | ErrorEnvelope {
  ...
  if (value.status === "success" && isRecord(value.data)) return value as unknown as SuccessEnvelope<T>;
...
export function CommunicationDashboard({ snapshot }: { snapshot: DashboardSnapshot }) {
  ...
  const selectedConversation = useMemo(() => currentSnapshot.conversations.find((conversation) => conversation.conversationId === selectedConversationId) ?? currentSnapshot.conversations[0], [selectedConversationId, currentSnapshot.conversations]);
  ...
  if (!selectedConversation) return <section className="empty-panel"><h2>No conversations yet</h2><p>Connect a LINE, X, or Instagram adapter webhook to start building a person-scoped inbox.</p></section>;
  ...
  return (
    <div className="dashboard-grid">
      ...
      <section className="panel reply-panel" aria-label="reply safety">
        <div className="panel-heading"><p className="eyebrow">Reply Draft</p><h2>Review before send</h2></div>
        {selectedConversation.replyDraft ? <div className="draft-editor"><label htmlFor="reply-draft-body">Draft body</label><textarea id="reply-draft-body" value={draftBody} onChange={(event) => setDraftBody(event.target.value)} rows={7} /><small>Purpose: {selectedConversation.replyDraft.purpose} / Hash: {selectedConversation.replyDraft.contentHash} / Status: {selectedConversation.replyDraft.status}</small><button className="secondary-button" disabled={isBusy || draftBody.trim().length === 0} onClick={() => runAction("Draft update", updateDraft)} type="button">Save draft</button></div> : <div className="draft-box"><p>No reply draft yet.</p></div>}
        <div className="gate-box" data-send-gate={sendUnlocked ? "unlocked" : "locked"}>
          <div className="gate-header"><div><p className="eyebrow">Safety Gate</p><strong>{sendUnlocked ? "Send unlocked" : "Send locked"}</strong></div><span className={sendUnlocked ? "status-pill success" : "status-pill warning"}>{sendUnlocked ? "ready" : "blocked"}</span></div>
          <p className="gate-reason">API readiness: {selectedConversation.safety.blockedReason ?? "ready"}</p>
          <div className="send-confirmation-card" aria-label="send confirmation summary"><p className="eyebrow">Send Confirmation</p><dl><div><dt>Person</dt><dd>{selectedConversation.displayName}</dd></div><div><dt>Conversation</dt><dd>{selectedConversation.conversationId}</dd></div><div><dt>Channel</dt><dd>{selectedConversation.channel}</dd></div><div><dt>Draft hash</dt><dd>{selectedConversation.replyDraft?.contentHash ?? "draft required"}</dd></div></dl></div>
          <div className="check-list">{gateChecks.map((check) => <label key={check.id} className="check-row" htmlFor={check.id}><input id={check.id} type="checkbox" checked={check.checked} onChange={(event) => check.setChecked(event.target.checked)} /><span>{check.label}</span></label>)}</div>
          <div className="action-row"><button className="secondary-button" disabled={isBusy || !selectedConversation.replyDraft} onClick={() => runAction("SafetyCheck", runSafetyCheck)} type="button">Run SafetyCheck</button><button className="send-button" disabled={isBusy || !sendUnlocked} onClick={() => runAction("Send", sendReply)} type="button">Send reply</button></div><p className="action-status" aria-live="polite">{actionStatus}</p>
        </div>
        <div className="send-history-card" aria-label="send decision history"><div className="history-heading"><p className="eyebrow">Send History</p><strong>{sendDecisions.length} decisions</strong></div>{sendDecisions.length === 0 ? <p className="history-empty">No send decision recorded for this draft.</p> : <ul>{sendDecisions.map((decision) => <li key={decision.sendDecisionId}><span>{decision.channel} / {decision.deliveryMode}</span><small>SafetyCheck {decision.safetyCheckId} / Hash {decision.contentHash} / {decision.decidedAt}</small><small>Adapter {decision.adapterReference}</small></li>)}</ul>}</div>
      </section>
      <section className="panel operations-panel" aria-label="adapter and AI platform status"><div className="panel-heading"><p className="eyebrow">Operations</p><h2>Adapters and AI tasks</h2></div><div className="status-grid">{currentSnapshot.adapterStates.map((adapter) => <div className="status-row" key={adapter.channel}><span>{adapter.channel}</span><strong>{adapter.status}</strong><small>{adapter.ossReference}</small></div>)}</div><div className="ai-task-list">{currentSnapshot.aiTasks.map((task) => <div className="ai-task" key={task.operation}><strong>{task.operation}</strong><span>{task.boundary}</span></div>)}</div></section>
    </div>
  );
}

function InsightList({ title, items }: { title: string; items: string[] }) { return <div className="insight-list"><h3>{title}</h3>{items.length === 0 ? <p>No records.</p> : <ul>{items.map((item) => <li key={item}>{item}</li>)}</ul>}</div>; }
